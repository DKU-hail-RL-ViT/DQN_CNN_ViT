...
parser.add_argument('--n-episodes', type=int, default=int(1e5), metavar='NE',
...
for j in range(1):
    ...
    for i_episode in tqdm(range(args.n_episodes)):
        ...
        for t in count():
            ...
            state_variables, reward, done, truncated, info = env.step(action)
            ...
            if done:
                ...
                wandb.log(log)
                ts.append(t+1)
                # print length of the episode
                print('Episode: ', i_episode, 'Length: ', t + 1)

                # print 1% 5% 10% 90% 95% 99% of the ts and mean of ts
                # print('1%: ', np.percentile(ts, 1), '5%: ', np.percentile(ts, 5), '10%: ', np.percentile(ts, 10),
                #         '90%: ', np.percentile(ts, 90), '95%: ', np.percentile(ts, 95), '99%: ', np.percentile(ts, 99),
                #         'mean: ', np.mean(ts))
                from scipy.stats import norm
                # print normal distribution z score with 2.5% 5% 10% 90% 95% 97.5% of the ts and mean of ts
                print('2.5%: ', norm.ppf(0.025, loc=np.mean(ts), scale=np.std(ts)), '5%: ', norm.ppf(0.05, loc=np.mean(ts), scale=np.std(ts)), '10%: ', norm.ppf(0.1, loc=np.mean(ts), scale=np.std(ts)),
                        '90%: ', norm.ppf(0.9, loc=np.mean(ts), scale=np.std(ts)), '95%: ', norm.ppf(0.95, loc=np.mean(ts), scale=np.std(ts)), '97.5%: ', norm.ppf(0.975, loc=np.mean(ts), scale=np.std(ts)),
                        'mean: ', np.mean(ts))
